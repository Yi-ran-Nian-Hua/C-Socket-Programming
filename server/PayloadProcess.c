...
struct Payload** ProcessSendData(const char* data, const uint16_t op, const uint16_t shiftNumber)
{
    ...
    struct Payload** payloads = (struct Payload**)malloc((groupTime + 1) * sizeof(struct Payload*) + 1); // 分配存储指针的数组内存
    if (payloads == NULL) {
        // 内存分配失败
        return NULL;
    }
    ...
    for (int i = 0; i < groupTime; i++)
    {
        ...
        if(i!=groupTime - 1) // 对于第一组到倒数第二组
        {
            ...
            BuildPayload(&data[i * (int)10e7], op, shiftNumber, payloads[i], (int)10e7); // 送入符合长度的数据进行载荷构造
...
{
    /* 首先先将接收到的数据从网络序转换为主机序 */
    payload->length = ntohl(payload->length);
    payload->shift = ntohs(payload->shift);
    payload->op = ntohs(payload->op);

    uint32_t strLength = payload->length - 2 - 2 - 4;
    /* 之后开始进行数据处理 */
    if(payload->op == 0)
    {
        //进行加密的代码
        *data = (char*) malloc(sizeof(char) * strLength + 1);
        if(*data == NULL)
        {
            fprintf(stderr, "Malloc failed.\n");
        }
        strncpy(*data, Encryption(payload->data, payload->shift), strLength);
    }
    else if(payload->op == 1)
    {
        ...
        *data = (char*) malloc(sizeof(char) * strLength + 1);
        ...
        strncpy(*data, Decryption(payload->data, payload->shift), strLength);
    }
    else
    {
        fprintf(stderr, "Invalid op code!(Only accept 0 and 1)\n");
    }
